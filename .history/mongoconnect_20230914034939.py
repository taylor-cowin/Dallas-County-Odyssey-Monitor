...
def get_latest():
    try:
        last_result = db_connect("local").find_one(sort=[('run_time', pymongo.DESCENDING)])
        #Strip the ID field
        del last_result["_id"]
...
def get_last_down():
    last_down = db_connect("local").find_one({"result": "DOWN" || "result": "D" || "result": "0"},sort=[('run_time', pymongo.DESCENDING)])
