...
def crash_checker():
    ...
    if last_entry["result"] == "DOWN" or last_entry["result"] == "D" or last_entry["result"] == 0:
        ...
        #If the last outage was after the last logged outage start time, then we started the script during an unlogged outage
        #TODO #NEED TO MAKE AN EXCEPTION FOR FIRST RUN DURING OUTAGE WITH NO DB BUILT YET
        if last_down != None and last_logged_outage["start_time"] > last_down["run_time"]:
